...
# carries.
#
# This is the half of pin hygiene that needs the network, so it is NOT in
# tests/ci-hygiene.sh (which must stay hermetic). That script enforces the offline
# half: every pin is a 40-hex SHA with an exact vX.Y.Z comment.
#
